...
export class Declare extends Instruction{
  ...
  execute(symbolTable: SymbolTable): any {
    ...
    if(this.ids.length > 1 && this.value){
      throw new Error(` sintáctico, no puede asignar un valor a una declaración multiple Linea: ${this.line} Columna: ${this.column}`);
    }
    ...
    if(this.ids.length == 1){
      ...
      if(isInTable){
        throw new Error(` semántico, nombre de variable definido anteriormente Linea: ${this.line} Columna: ${this.column}`);
      }
      ...
      if(this.value){
        ...
        if(!operation){
          throw new Error(` semántico, Algo ocurrió con la operación a asignar Linea: ${this.line} Columna: ${this.column}`);
        }

        if(this.variableType != operation.variableType){
          throw new Error(` semántico, Los tipos no coinciden Linea: ${this.line} Columna: ${this.column}`);
        }
